...
resource "google_compute_instance" "vm_instance" {
  ...
  metadata = {
    ssh-keys = "ssh_user:${file("/home/pasindud/.ssh/ssh_key.pub")}"
  }
  ...
  provisioner "remote-exec" {
    ...
    connection {
      ...
      private_key = "${file("/home/pasindud/.ssh/ssh_key")}"
  ...
  provisioner "local-exec" {
    command = "ansible-playbook --extra-vars='{\"region\": ${each.value.region}, \"zone\": ${each.value.zone}, \"vm_name\": ${each.value.vm_name} }' -i '${self.network_interface[0].access_config[0].nat_ip},' ${var.playbook}"
  }
